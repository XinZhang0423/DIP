...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1, 'input_size': (1, 128, 128), 'pool_size': None,
...
class Block_attention(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, h=14, w=8):
        super().__init__()
        num_heads= 2 # 4 for tiny, 6 for small and 12 for base
        self.norm1 = norm_layer(dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=128, patch_size=16, in_chans=1, embed_dim=64):
        super().__init__()
...
class SpectFormer(nn.Module):
    ...
    def __init__(self, img_size=128, patch_size=16, in_chans=3, num_classes=1, embed_dim=64, depth=12,
        ...
        #我自己加的
        self.conv = nn.Conv2d(embed_dim,num_classes,1,1,1)
