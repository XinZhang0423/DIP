...
class UpSample(nn.Module):
    def __init__(self, input_resolution, in_channels, scale_factor):
        ...
        if self.factor == 2:
            self.up_b = nn.Sequential(nn.Upsample(scale_factor=scale_factor, mode='bilinear', align_corners=False),
                                      nn.Conv2d(in_channels, in_channels // 2, 1, stride=1, padding=0, bias=False))
        elif self.factor == 4:
            self.up_b = nn.Sequential(nn.Upsample(scale_factor=scale_factor//2, mode='bilinear', align_corners=False),
                                      nn.ReplicationPad2d(1),
                                      nn.Conv2d(in_channels, in_channels, 3, stride=(1, 1), padding=0),
    ...
    def forward(self, x):
        ...
        out = x_b
        # out = self.conv(torch.cat([x_b, x_b], dim=1))
